...
public class PoseEstimator {
  ...
  private static PoseEstimator my_instance = null;

  public static synchronized PoseEstimator getInstance() {
    if (my_instance == null) {
      my_instance = new PoseEstimator(VecBuilder.fill(0.005, 0.005, 0.0005));
    }
    return my_instance;
  }
  ...
  /** Resets the odometry to a known pose. */
  public void mixPose(Pose2d visionPose, Double k) {
    double x = (getLatestPose().getX() * (1 - k)) + (visionPose.getX() * k);
    double y = (getLatestPose().getY() * (1 - k)) + (visionPose.getY() * k);
    Rotation2d theta =
        new Rotation2d(
            (getLatestPose().getRotation().getRadians() * (1 - k))
                + (visionPose.getRotation().getRadians() * k));
    basePose = new Pose2d(x, y, theta);
    updates.clear();
    update();
  }
  ...
  /** Records a new drive movement. */
  public void addVisionData(Pose2d visionPose) {
    mixPose(visionPose, 0.1);
  }
